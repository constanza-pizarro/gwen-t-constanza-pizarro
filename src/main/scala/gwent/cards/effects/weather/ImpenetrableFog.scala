...
case class ImpenetrableFog() extends AbstractWeatherEffect {
  override def apply(self: Card, target: Board): Unit = {
    ...
    applyEffect(target.player1.section.rangedCombatZone, target.player2.section.rangedCombatZone)
  }
  override def undo(target: Board): Unit = {
    unapplyEffect(target.player1.section.rangedCombatZone, target.player2.section.rangedCombatZone)
  }
}
